...
layui.define(["jquery", "miniMenu", "element", "miniTab", "miniTheme"], function (exports) {
    ...
    var miniAdmin = {
        ...
        render: function (options) {
            ...
            window.get(options.iniUrl, function (data) {
                if (!window.checkError(data) && data.errcode !== 4012 && data.errcode !== 4013) {
                    if (!data.errmsg) {
                        miniAdmin.error(data.errmsg);
                    } else {
                        miniAdmin.error('菜单获取出错');
                    }
                } else {
                    miniAdmin.renderLogo(data.data.logoInfo);
                    miniAdmin.renderClear(options.clearUrl);
                    miniAdmin.renderHome(data.data.homeInfo);
                    miniAdmin.renderAnim(options.pageAnim);
                    miniAdmin.listen();
                    miniMenu.render({
                        menuList: data.data.menuInfo,
